...
function LocalDataProvider(model, filterFilter, orderByFilter, limitToFilter, defaultViewSettings) {
	...
	function updateFilters() {
		...
		if (viewSettings.orderBy) {
			var reverse=false;
			if(viewSettings.order == "DESC"){
				reverse=true;
			}
			viewModel = orderByFilter(viewModel, viewSettings.orderBy, reverse);
		}
	...
	this.orderBy = function (expression, reverse) {
		...
		viewSettings.orderBy = expression;
		if(reverse){
			viewSettings.order = "DESC";
		}else{
			viewSettings.order = "ASC";
		}
		updateFilters();
	};
...
function ServerDataProvider(requestService, $timeout, defaultViewSettings, debounceTime) {
	...
	//分页的过滤器
	_this.updateFilters = function(resourceUrl,v,e) {
		if (e && e == 'no_request') {
			return;
		}
		if (resourceUrl) {
		...
		//删除其中的某些属性
			...
			requestService.post(_this.url, _this.viewSettings, true).then(function(d) {
				_this.data = d;
				if(_this.onDataChange!=null) _this.onDataChange(_this.data);
            },function(error) {
...
angular.module("lightGrid").directive("lgPager", function () {
	...
	return {
		...
		replace:true,
		template: 
			"<div style='height:40px;'><div class='page-size' style='height:30px;float:right'><select class='form-control' ng-options='size for size in pageSizes' ng-model='pageSize'></select></div>"+
	...
	return {
		...
		template: 
			"<div style='height:40px;'><div class='page-size' style='height:35px;float:right'><select class='form-control' ng-options='size for size in pageSizes' ng-model='pageSize'></select><div class='all_num' style='float:right;height:35px;line-height:35px;margin-right:50px;font-size:16px;'><span style='float:left;'>总条数:</span><p style='float:left;' ng-model='totalItemCount'>{{totalItemCount}}</p></div></div>"+
		"<div class='pager' style='float:right'>" +
			"<button ng-disabled='isFirst'style='float:left' class='first' ng-click='goToFirst()'><i class='glyphicon glyphicon-step-backward kj'></i></button>" +
			"<button ng-disabled='isFirst' style='float:left' class='previous' ng-click='goToPrevious()'><i class='glyphicon glyphicon-chevron-left kj'></i></button>" +
			"<div class='pager-summary'style='float:left;height:35px;line-height:35px'>第<input type='text' ng-keyup='jumPage(currentPage)' class='numberS' ng-model='currentPage' />页/共 {{pageCount}}页</div>" +
			"<button ng-disabled='isLast' style='float:left' class='next' ng-click='goToNext()'><i class='glyphicon glyphicon-chevron-right kj'></i></button>" +
			"<button ng-disabled='isLast' style='float:left' class='last' ng-click='goToLast()'><i class='glyphicon glyphicon-step-forward kj'></i></button>" +
			"</div></div>" ,
		link: function pagerLink($scope) {
			var DEFAULT_PAGE_SIZE_OPTIONS = "3,20,30,50,100";
			...
			//跳转页面
			$scope.jumPage = function(pageNumber){
				var currKey=event.keyCode;
				/*if((currKey >= 96 && currKey <= 105) || currKey > 96 ||currKey <= 105){*/
				if(currKey == 13){
					//如果当前页小于等于1那就是1，如果大于等于总页数，那就是总页数
					if (pageNumber <= 1) {
						pageNumber = 0;
			...
			$scope.goToNext = function () {
				if (typeof $scope.currentPage == 'string') {
...
angular.module("lightGrid").directive("lgToggleExpandedRow", ["$timeout", function ($timeout) {
	...
	return {
		...
		link: function toggleExpandedRowLink($scope, $elem, $attrs, $ctrl) {
			...
			$elem.on("click", function () {
				$timeout(function () {
					if($scope.topshowRoles==null||$scope.topshowRoles==$scope.showRoles){
						$scope.topshowRoles=$scope.showRoles;
