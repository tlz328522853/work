  ...
  .directive('treeSelection', ['$rootScope',function($rootScope) {
    return {
      ...
      controller: function($scope,$rootScope) {
    	/*this.unSelected =function (level,isOrg){
    		
    		if($scope.onSelect){
    			$scope.onSelect({"level":level,"isOrg":isOrg});
       	  	}
...
//        scope.level.indeterminate = false;
        ...
        var template =  '<i class="fa" ng-show="level.hasChildren||(level.children && level.children.length > 0)"  ng-class="{true:\'glyphicon glyphicon-minus\', false:\'glyphicon glyphicon-plus\', undefined:\'glyphicon glyphicon-plus\'}[level.mark.open]" ng-click="click(level)"></i>' +
                        ...
                        '</div>' +
                        '<span class="cb-label">{{level.label}}</span>' +
